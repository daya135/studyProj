...
public class PrimeNumber {
	...
	//幼稚园版本
	public static boolean validate1(long n) {
		...
		for (int i = 2; i < n; i++) {	//傻傻的遍历：“除了1和其本身，没有任何数能整除它”
			if (n % i == 0) {
				...
				Print.print("false 运行时间：" + (timeAfter - timeBefore));
				return false;
			}
		}
		timeAfter = System.currentTimeMillis();
		Print.print("true 运行时间：" + (timeAfter - timeBefore));
		return true;
	}
	
	//小学生版本
	public static boolean validate2(long n) {
		timeBefore = System.currentTimeMillis();
		if (n <= 1) return false;
	...
	//中学生
	public static boolean validate3(long n) {
		timeBefore = System.currentTimeMillis();
	...
	//高中生
	public static boolean validate4(long n) {
		timeBefore = System.currentTimeMillis();
		if (n == 2) return true;	//先把2放一边
		if (n <= 1 || n % 2 == 0) return false;	//再排除偶数
	...
	public static boolean validate5(long n) {
		int[] prime = new int[10000000];	//表的大小为一百万
		timeBefore = System.currentTimeMillis();
		prime[1] = 0;
		for(int i = 2; i <= n ; i++){
			prime[i] = 1;
